 ...
 * 
 * Describe: The javascript boot file of account page (repay.plan).
 * 
	...
	boot.req = function (jquery) {
		...
		require([
			...
			SJ(function ($) {
				...
				$('.repayPlan').find('tr').last().find('td').addClass('lastRepayPlan');
                
                
                
				/**
				 * if repaid, font color is light gray.
				 */
                
				$('.repayStatus').each(function () {
                    
					if ($(this).text() === "已还完") {
                        
						$(this).parent('tr').children('td').addClass('repaid');
